...
import { setGroupId, setGroupParticipent } from "../../store/slices/groupSlice";
...
const GroupChatBox = ({ navigation }) => {
  const user = useSelector(state => state.user)
  const group = useSelector(state => state.group)
  // console.log({ cccccc: group.groupData.conversationId });
  ...
  // console.log({conversationId: group?.groupData?._id});
  useEffect(() => {
    if (socketConnection) {
      setChatLoader(true)
      ...
      const data = {
        ...
        conversationId: group?.groupData?.conversationId
      }
      socketConnection.emit('group-message-page', data)
      let datas = { conversationId: group?.groupData?.conversationId, msgByUserId: user._id }
  ...
  const UploadImage = async (path, mediaType) => {
    ...
    formData.append('conversationId', group?.groupData?._id)
    ...
    await axios.post(`${basepath}user-auth/images-upload`, formData, {
      ...
      console.log({ err: err.message }, '111');
    })
  }
  ...
  useEffect(() => {
    const getGroupInfo = async () => {
      await axios.get(`${basepath}group/get-group-info?groupId=${group.groupId}`, {
        headers: {
          Authorization: `Bearer ${user.token}`
        }
      }).then((res) => {
  ...
  // console.log({ participent: group.groupParticipent });

  const handleSendMessage = () => {
    // if (message.text || message.imageUrl || message.videoUrl) {
    if (socketConnection) {
      let reqData = {}
      const updatedParticipants = [...group.groupParticipent, user._id];

      if (group.groupParticipent.length > 0) {
        switch (true) {
          case mediaType === 'image':
            reqData = {
              ...
              groupId: group.groupId,
              participent: updatedParticipants
            }
            break;
        ...
        switch (true) {
          ...
          case mediaType === 'video':
            reqData = {
              sender: user?._id,
              text: text,
              videoUrl: file !== null ? message?.videoUrl || file : "",
              msgByUserId: user?._id,
              groupId: group.groupId,
              conversationId: group.groupData.conversationId
      ...
      socketConnection.emit('group-message', reqData)
  ...
  const handleClearChat = () => {
    if (socketConnection) {
      socketConnection.emit('clear-conversation', group.groupData.conversationId);
    }
  ...
  // console.log({groupId:groupInfo?._id});
  const handleNavigateGroupInfo=()=>{
    dispatch(setGroupId(groupInfo))
  ...
  const handleLeaveGroup = async () => {
    await axios.patch(`${basepath}/group/exist-from-group`,
      ...
      console.log({err:err.message},'leave error');
    })
  }
  // console.log({user});
  return (
    <SafeAreaView style={[styles.notiParent, user.isDarkMode && { backgroundColor: "#0D142E" }]}>
      ...
      {top.main &&
        ...
        >
          ...
          <View style={styles.headerAllIcon}>
            <TouchableOpacity style={styles.headerTitleIcon} onPress={() => handleNavigateGroupInfo()}>
              <View style={styles.headerAvatar}>
                <Image
                  style={{width:45,height:45,borderRadius:30,borderWidth:1,borderColor:"#9B7BFF",overflow:"hidden",marginLeft:4}}
                  resizeMode="cover"
                  source={{uri:groupInfo?.groupProfile}}
                />
              ...
              <Text style={[styles.ashishRajput, styles.amText]}>
                {!groupInfo?.title ?
                  ...
                  :
                  `${groupInfo?.title.substring(0, 10)}...`
                }
              </Text>
                          ...
                          <View style={styles.chatMessageLayout1}>
                            ...
                            >
                              ...
                              }
                              {
                                item?.docUrl !== '' &&
                                <TouchableOpacity>
                                  <Image source={require('../../assets/acount.png')} />
                                  <Text style={{ color: "#000" }} >{item?.docUrl}</Text>
                                </TouchableOpacity>
                              }
                              <Hyperlink
      ...
      >
        <View style={[styles.userModel, user.isDarkMode && { backgroundColor: "#000", borderColor: "#fff", borderWidth: 1 }]} >
          <TouchableOpacity style={styles.modelitem} onPress={()=>handleNavigateGroupInfo()} >
            <Text style={[styles.modeltext, user.isDarkMode && { color: "#fff" }]}>Group info</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.modelitem} onPress={()=>navigation.navigate('Group Media')}>
            <Text style={[styles.modeltext, user.isDarkMode && { color: "#fff" }]}>Group media</Text>
          </TouchableOpacity>
          ...
          {/* <TouchableOpacity style={styles.modelitem}>
            <Text style={[styles.modeltext, user.isDarkMode && { color: "#fff" }]}>Ban</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.modelitem}>
            <Text style={[styles.modeltext, user.isDarkMode && { color: "#fff" }]}>Report</Text>
          </TouchableOpacity> */}
          <TouchableOpacity style={styles.modelitem} onPress={()=>handleLeaveGroup()} >
            <Text style={[styles.modeltext, user.isDarkMode && { color: "#fff" }]}>Exit Group</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.modelitem} onPress={() => handleClearChat()} >
            <Text style={[styles.modeltext, user.isDarkMode && { color: "#fff" }]}>Clear Chat</Text>
          </TouchableOpacity>
          {/* {
            isActiveConvUser === 0 ?
