...
import { setSMS } from "../../store/slices/userSlice";
...
import { setChanelData } from "../../store/slices/chanelSlice";
...
const ChanelChatBox = ({ navigation }) => {
  const user = useSelector(state => state.user)
  const chanel = useSelector(state => state.chanel)
  // console.log({ chanel: chanel.chanelData});
  // const {participent}=route.params;
  const dispatch = useDispatch()
  // console.log({ user: user.isDarkMode });
  ...
  useEffect(() => {
    if (socketConnection) {
      ...
      const data = {
        ...
        conversationId: chanel?.chanelData?.conversationId
      }
      socketConnection.emit('chanel-message-page', data)
      let datas = { conversationId: chanel.chanelData?.conversationId, msgByUserId: user._id }
      ...
      socketConnection.on('message', (data) => {
        // console.log({data:data[0].msgByUserId})
        // playSoundIn()
        setAllMessage(data)
        setChatLoader(false)
      })
  ...
  const UploadImage = async (path, mediaType) => {
    ...
    formData.append('conversationId', chanel?.chanelData?.conversationId)
    ...
    await axios.post(`${basepath}user-auth/images-upload`, formData, {
      ...
      console.log({ err: err.message }, 'images upload error');
    })
  }
  ...
  // console.log({chanel:chanel.chanelId});
  const [chanelInfo, setChanelInfo] = useState(null)
  const [isChanelAdmin, setIsChanelAdmin] = useState(false)
  console.log({ isChanelAdmin, chanelInfo });
  useEffect(() => {
    const getGroupInfo = async () => {
      await axios.get(`${basepath}chanel/get-chanel-info?chanelId=${chanel.chanelId}`, {
        headers: {
          Authorization: `Bearer ${user.token}`
        }
      }).then((res) => {
        console.log({ res: res?.data?.data?.chanelAdmin });
        let chanelAdmin = res?.data?.data?.chanelAdmin?.some((item) => item === user._id)
        setIsChanelAdmin(chanelAdmin)
        setChanelInfo(res.data.data)
      }).catch((err) => {
        console.log({ err });
      })
    }
  ...
  const handleExitChanel = async () => {
    await axios.patch(`${basepath}chanel/exist-from-chanel`, {chanelId:chanelInfo._id}, {
      headers: {
        Authorization: `Bearer ${user.token}`
      }
    }).then((res)=>{
  ...
  const handleNavigateChanelInfo = () => {
    dispatch(setChanelData(chanelInfo))
    navigation.navigate('Chanel Info')
  }

  console.log({ participent: [...chanel.chanelParticipent, user._id] });

  const handleSendMessage = () => {
    // if (message.text || message.imageUrl || message.videoUrl) {
    if (socketConnection) {
      let reqData = {}
      let updateParticipent = [...chanel.chanelParticipent, user._id]
      console.log({ updateParticipent });
      if (chanel?.chanelParticipent?.length > 0) {
        switch (true) {
          case mediaType === 'image':
            reqData = {
              ...
              // videoUrl: file !== null ? message?.videoUrl : "",
          ...
          case mediaType === 'video':
            reqData = {
              ...
              text: text,
              // imageUrl: file !== null ? file?.mediaurl || file : '',
              videoUrl: file !== null ? message?.mediaurl || file : "",
              msgByUserId: user?._id,
              chanelId: chanel.chanelId,
              participent: updateParticipent
            }
          ...
          case mediaType === 'audio':
            reqData = {
              ...
              text: text,
              // imageUrl: file !== null ? file?.mediaurl || file : '',
              audioUrl: file !== null ? message?.mediaurl || file : "",
              msgByUserId: user?._id,
              chanelId: chanel.chanelId,
              participent: updateParticipent
            }
          ...
          case mediaType === 'doc':
            reqData = {
              ...
              text: text,
              // imageUrl: file !== null ? file?.mediaurl || file : '',
              docUrl: file !== null ? message?.mediaurl || file : "",
              msgByUserId: user?._id,
              chanelId: chanel.chanelId,
              participent: updateParticipent
            }
        ...
        switch (true) {
          case mediaType === 'image':
            reqData = {
              ...
              msgByUserId: user?._id,
              chanelId: chanel.chanelId,
              conversationId: chanel?.chanelData?._id
          ...
          case mediaType === 'video':
            reqData = {
              sender: user?._id,
              text: text,
              // imageUrl: file !== null ? file?.mediaurl : '',
      ...
      socketConnection.emit('chanel-message', reqData)
      dispatch(setGroupParticipent([]))
      playSoundOut()
      setText('')
      setFile(null)
      dispatch(setSMS(''))
  ...
  const filterdata = allMessage?.filter((item) => item.text.toLowerCase().includes(searchkey?.toLowerCase()))



  console.log({ chanelInfo });
  return (
    <SafeAreaView style={[styles.notiParent, user.isDarkMode && { backgroundColor: "#0D142E" }]}>
      ...
      {top.main &&
        ...
        >
          ...
          <View style={styles.headerAllIcon}>
            <TouchableOpacity style={styles.headerTitleIcon} onPress={() => handleNavigateChanelInfo()}>
              <View style={styles.headerAvatar}>
                <Image
                  style={{ width: 45, height: 45, borderRadius: 30, borderWidth: 1, borderColor: "#9B7BFF", overflow: "hidden", marginLeft: 4 }}
                  resizeMode="cover"
                  source={{ uri: chanelInfo?.chanelProfile }}
                />
              ...
              <Text style={[styles.ashishRajput, styles.amText]}>
                {chanelInfo?.title ? `${chanelInfo?.title.substring(0, 10)}...` : `Loading...`}
              </Text>
      ...
      }
      {
        ...
        <View style={styles.searchTop} >
          <TextInput
            placeholderTextColor={user.isDarkMode ? "#aaa" : "#999"}
            placeholder="Search Here ..."
            style={{ borderRadius: 25, borderWidth: 2, borderColor: user.isDarkMode ? '#fff' : '#ccc', paddingLeft: 20, backgroundColor: user.isDarkMode ? '#222' : '#E0E0E0', color: user.isDarkMode ? '#fff' : '#000' }}
          ...
          <TouchableOpacity
            style={{ top: 5, right: 20, position: "absolute", width: 50, zIndex: 1000 }}
            onPress={() => {
              setSearchKey('');
            ...
            allMessage?.length > 0 ?
              filterdata.length > 0 ?
                filterdata.map((item, index) => {
                  console.log({ docUrl: item?.docUrl });
                  return (
                    item.msgByUserId._id === user._id ?
                      <Pressable key={index} style={[styles.chatMessage1]} onPress={() => { isChanelAdmin && handleSelect(item) }} >
                            ...
                            >
                              ...
                              }
                              {
                                ...
                                <TouchableOpacity style={
                                  ...
                                  onPress={() => { isChanelAdmin && downloadImageRemote(item?.docUrl) }}
                                >
                              ...
                              {/* <Text style={[styles.helloHowsEveryone1, styles.helloTypo]}>
                            ...
                            >
                              {
                                item?.imageUrl !== '' &&
                                <View>
                                  <Image source={require('../../assets/acount.png')} style={{ height: 100, height: 120, borderRadius: 15 }} />
                                  {/* <Text style={{color:"#000"}} >Image</Text> */}
                                </View>
                              }
                              <Text style={[styles.helloHowsEveryone, styles.helloTypo1]}>
        ...
        enableedit ?
          <View style={[styles.bottomIcons, enableedit && { backgroundColor: "#A9C2D0" }]} >
            <View style={[styles.typeMessageParent, enableedit && { width: "100%" }]}>
              ...
              {/* <TouchableOpacity style={[styles.fluentemoji24RegularIcon]} onPress={() => setEmoji(!emoji)} >
                <Image
                  style={{ tintColor: "white" }}
                  resizeMode="cover"
                  source={require("../../assets/fluentemoji24regular.png")}
                />
              </TouchableOpacity> */}
      ...
      >
        <View style={[styles.userModel, user.isDarkMode && { backgroundColor: "#000", borderColor: "#fff", borderWidth: 1 }]} >
          <TouchableOpacity style={styles.modelitem} onPress={() => handleNavigateChanelInfo()}  >
            <Text style={[styles.modeltext, user.isDarkMode && { color: "#fff" }]}>Info</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.modelitem} onPress={() => navigation.navigate('Chanel Media')} >
            <Text style={[styles.modeltext, user.isDarkMode && { color: "#fff" }]}>Media</Text>
          </TouchableOpacity>
          ...
          <TouchableOpacity style={styles.modelitem} onPress={() => handleExitChanel()}  >
            <Text style={[styles.modeltext, user.isDarkMode && { color: "#fff" }]}>Exit Signal</Text>
          </TouchableOpacity>
      ...
      >
        <View style={[styles.readcontainer, user.isDarkMode && { backgroundColor: "#000", borderColor: "#fff", borderWidth: 1 }]}>
          ...
          <TouchableOpacity onPress={() => setRead(!read)} style={{ position: "absolute", top: 7, right: 8 }} >
            <Image source={require('../../assets/iconoircancel.png')} style={{ width: 35, height: 35, tintColor: 'blue', borderRadius: 30, borderWidth: 2, borderColor: "blue" }} />
          </TouchableOpacity>

          <View style={{ backgroundColor: "#D9E6EB", borderRadius: 10, padding: 8, marginTop: 50 }}>
            {
              selectsms?.imageUrl !== '' &&
