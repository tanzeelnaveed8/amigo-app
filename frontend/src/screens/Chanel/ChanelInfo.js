...
const ChanelInfo = ({ navigation }) => {
  const user=useSelector(state=>state.user)
  const chanel=useSelector(state=>state.chanel.chanelData)
  const [selectUser, setSelectUser] = useState('')
  const admin = chanel?.chanelAdmin?.some((item) => item === user._id)
  ...
  const isGroupAdmin = (userId) => {
    isAdmin = chanel.chanelAdmin?.some((item) => item === userId)
    return isAdmin;
  }
  ...
  const handleExitChanel = async () => {
    await axios.patch(`${basepath}chanel/exist-from-chanel`, {chanelId:chanel._id}, {
      headers: {
  ...
  const handleKickUser = async () => {
    await axios.patch(`${basepath}chanel/remove-from-chanel`, {
      chanelId: chanel._id,
  ...
  const handleMakeAdmin = async () => {
    await axios.patch(`${basepath}/chanel/add-member-in-chanel-as-an-admin`,
      {
        chanelId: chanel._id,
        userId: selectUser._id
      },
    ...
    <SafeAreaView style={[styles.container, user.isDarkMode && { backgroundColor: "#0D142E" }]} >
      ...
      <View style={styles.profileContainer}>
        <View style={styles.profile}>
          <Image
            style={{height:50,width:50,borderRadius:30,borderWidth:2,borderColor:"#9B7BFF"}}
            resizeMode="cover"
            source={{uri:chanel?.chanelProfile}}
          />
        </View>
        <Text style={{ fontSize: 22, color: 'white', paddingVertical: 10, fontWeight: "700" }} >{chanel.title}</Text>
        <View style={styles.mediacontainer} onPress={() => navigation.navigate('Call Video')} >
          ...
          <TouchableOpacity style={styles.mediaitem} onPress={() => navigation.navigate('Chanel Setting')} >
      ...
      {
          chanel.participants?.map((item, index) => {
            return (
