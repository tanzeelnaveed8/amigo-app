...
import { ArrowLeft, Edit2, Trash2, Save, X, QrCode, Share2, Download, CheckCircle, ChevronRight, Ghost, Users, Info, Bell, Shield, Megaphone, LogOut, Camera, Link } from 'lucide-react';
...
interface AmigoSignalSettingsScreenProps {
  onBack: () => void;
  chatId?: string;
  name?: string;
  description?: string;
...
export const AmigoSignalSettingsScreen = ({ 
  onBack, 
  chatId = 'signal-1',
  name = 'Tech News Daily',
  description = 'Your daily dose of the latest technology news, breakthroughs, and insights. 🚀'
}: AmigoSignalSettingsScreenProps) => {
  const { amigoThemeMode: themeMode } = useSessionStore();
  
  // Signal Info States
  const [signalName, setSignalName] = useState(name);
  const [isEditingName, setIsEditingName] = useState(false);
  const [signalDesc, setSignalDesc] = useState(description);
  const [isEditingDesc, setIsEditingDesc] = useState(false);
  const [websiteLink, setWebsiteLink] = useState('https://technews.daily');
  const [isEditingLink, setIsEditingLink] = useState(false);
  
  // Feature States
  const [showQrModal, setShowQrModal] = useState(false);
  const [deleteSignalDialogOpen, setDeleteSignalDialogOpen] = useState(false);
  const [copyFeedback, setCopyFeedback] = useState(false);
  const [avatar, setAvatar] = useState('https://images.unsplash.com/photo-1599305445671-ac291c95aaa9?w=400&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Mnx8bG9nb3xlbnwwfHwwfHx8MA%3D%3D');
  
  const isDarkMode = themeMode !== 'day';
  const themeColor = themeMode === 'ghost' ? '#9B7BFF' : themeMode === 'day' ? '#2563EB' : '#3B82F6';
  
  // Mock counts
  const subscriberCount = 12500;
  ...
  const handleShareQr = async () => {
    const shareData = {
        ...
        text: `Scan this QR code to join the signal "${signalName}".`,
        url: `amigo://signal/${chatId}` 
    };
    ...
    if (navigator.share) {
        ...
        // Fallback
        navigator.clipboard.writeText(`amigo://signal/${chatId}`);
  ...
  const handleSaveQr = async () => {
    try {
        const qrElement = document.getElementById('signal-qr-code');
        if (!qrElement) return;

        const ghostIconSize = 32;
        // Use Megaphone icon path for Signal
        const ghostSvg = `
          <svg xmlns="http://www.w3.org/2000/svg" width="${ghostIconSize}" height="${ghostIconSize}" viewBox="0 0 24 24" fill="none" stroke="${themeColor}" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
        ...
        const nameText = signalName || 'Signal';
        const nameSpacing = 3;
        ...
        // Subtitle: Scan to join signal
        ctx.font = 'bold 12px sans-serif';
        ctx.fillStyle = themeColor;
        
        const subtitleText = "SCAN TO JOIN SIGNAL";
  ...
  return (
    <div className={cn("flex flex-col h-screen w-full overflow-hidden relative transition-colors duration-300", themeClasses.bg)}>
      {/* Header */}
      <div className={cn("flex-none px-5 pt-4 pb-2 z-10 sticky top-0 backdrop-blur-md border-b", themeClasses.divider, themeClasses.headerBg)}>
        <div className="flex items-center justify-between mb-4">
          ...
          <span className={cn("font-bold text-lg", themeClasses.text)}>Signal Settings</span>
          <div className="w-10" />
        </div>
      </div>
      ...
      <div className={cn("flex-1 overflow-y-auto custom-scrollbar px-5 py-6 space-y-8", themeClasses.bg)}>
        ...
        <section className="flex flex-col items-center justify-center mb-6">
            <div className="relative group">
                <div className={cn(
                    ...
                    <img src={avatar} alt="Signal Avatar" className="w-full h-full object-cover" />
                </div>
            ...
            <p className={cn("mt-3 text-xs font-medium opacity-60", themeClasses.subtext)}>Tap to change signal icon</p>
        </section>
            ...
            <div className={cn("p-4 rounded-2xl border mb-4 space-y-4", themeClasses.card)}>
                ...
                <div className="space-y-2">
                    <div className="flex items-center justify-between">
                        <span className={cn("text-sm font-medium opacity-70", themeClasses.text)}>Signal Name</span>
                        {/* Read-only for non-admins usually, but enabling edit for demo */}
                        {!isEditingName ? (
                            <button 
                    ...
                    {isEditingName ? (
                        ...
                        <h2 className={cn("text-xl font-bold", themeClasses.text)}>{signalName}</h2>
                    )}
                </div>
                ...
                <div className="space-y-2">
                    ...
                    {isEditingDesc ? (
                        ...
                        <p className={cn("text-sm font-medium leading-relaxed", themeClasses.subtext)}>{signalDesc}</p>
                    )}
                </div>
                ...
                <div className="space-y-2">
                     <div className="flex items-center justify-between">
                        <span className={cn("text-sm font-medium opacity-70", themeClasses.text)}>Website</span>
                        {!isEditingLink ? (
                            <button 
                    ...
                    {isEditingLink ? (
                        <div className="relative">
                            <Link size={16} className={cn("absolute left-3 top-1/2 -translate-y-1/2 opacity-50", themeClasses.text)} />
                            <input 
                                type="text" 
                                value={websiteLink}
                                ...
                                autoFocus
                                className={cn("w-full pl-9 pr-3 py-2 rounded-xl border text-sm font-medium outline-none focus:ring-2 focus:ring-blue-500/50 transition-all", themeClasses.inputBg)}
                            />
                        </div>
                    ) : (
                        <div className="flex items-center gap-2">
                            <Link size={14} className={themeClasses.subtext} />
                            <a href={websiteLink} target="_blank" rel="noopener noreferrer" className={cn("text-sm font-medium hover:underline text-blue-500")}>
                                {websiteLink}
                            </a>
                        </div>
                    )}
            ...
            >
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col items-start">
                        <span className={cn("text-sm font-semibold", themeClasses.text)}>Signal QR Code</span>
                        <span className={cn("text-xs opacity-60", themeClasses.subtext)}>Scan to subscribe</span>
                    </div>
                </div>
        ...
        {/* Total Subscribers */}
        <section>
            <h3 className={cn("text-xs font-bold uppercase tracking-wider mb-3 opacity-60 ml-1", themeClasses.subtext)}>Community</h3>
            <div className={cn("p-4 rounded-2xl border", themeClasses.card)}>
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <div className={cn("p-2 rounded-xl", isDarkMode ? "bg-white/5" : "bg-gray-100")}>
                            <Users size={20} className={themeClasses.text} />
                        </div>
                        <span className={cn("font-medium", themeClasses.text)}>Total Subscribers</span>
                    </div>
                    <span className={cn("text-lg font-bold", themeClasses.text)}>
        ...
        <section>
            <h3 className={cn("text-xs font-bold uppercase tracking-wider mb-3 opacity-60 ml-1 text-red-500")}>Danger Zone</h3>
             <div className={cn("rounded-2xl border border-red-500/20 overflow-hidden", isDarkMode ? "bg-red-500/5" : "bg-red-50")}>
                 <button 
                    onClick={() => setDeleteSignalDialogOpen(true)}
                    className="w-full flex items-center gap-3 p-4 text-red-500 hover:bg-red-500/10 transition-colors text-left"
                >
                    <Trash2 size={20} />
                    <span className="font-medium">Delete Signal</span>
                </button>
             </div>
             <p className={cn("text-xs mt-2 ml-1 opacity-50", themeClasses.subtext)}>
                 Deleting a signal is permanent and cannot be undone. All messages and media will be lost.
             </p>
        </section>
        ...
        <Dialog open={showQrModal} onOpenChange={setShowQrModal}>
            <DialogContent className={cn("sm:max-w-sm border-0 p-0 overflow-hidden bg-transparent shadow-none")}>
                <DialogHeader className="sr-only">
                    <DialogTitle>Scan to Join Signal</DialogTitle>
                    <DialogDescription>
                        Scan this QR code to join {signalName}
                    </DialogDescription>
                </DialogHeader>
                <motion.div
                ...
                >
                    ...
                    <h3 className={cn("text-xl font-bold mb-1", themeClasses.text)}>Scan to Subscribe</h3>
                    <p className={cn("text-sm mb-6", themeClasses.subtext)}>{signalName}</p>
                                    ...
                                    <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
                                        <div className="relative">
                                            ...
                                            <div className={cn(
                                                ...
                                                <Megaphone size={24} style={{ color: themeColor }} strokeWidth={2.5} />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <p className={cn("text-xs text-center opacity-60 max-w-[200px] mb-6", themeClasses.subtext)}>
                        Share this QR code with others so they can join this signal instantly.
                    </p>
        ...
        {/* Delete Signal Dialog - Glowing Animation Style */}
        <AlertDialog open={deleteSignalDialogOpen} onOpenChange={setDeleteSignalDialogOpen}>
            <AlertDialogContent className={isDarkMode ? "bg-[#0A0A14] border-white/10 text-white max-w-md" : "bg-white text-gray-900 max-w-md"}>
               ...
               >
                  ...
                  <AlertDialogHeader className="text-center space-y-3">
                    <AlertDialogTitle className="text-xl">Delete Signal?</AlertDialogTitle>
                    <AlertDialogDescription className={isDarkMode ? "text-[#8B8CAD]" : "text-gray-500"}>
                      This action cannot be undone. This will permanently delete the signal and all its data.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter className="w-full mt-6 flex-col-reverse sm:flex-row gap-2">
                     ...
                     <AlertDialogAction 
                        onClick={() => { setDeleteSignalDialogOpen(false); onBack(); }} 
