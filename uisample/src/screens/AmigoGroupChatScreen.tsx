...
import { ArrowLeft, MoreVertical, Paperclip, Send, Moon, Ghost, Smile, Mic, Eye, CheckCircle2, Loader2, AlertCircle, FileText, Download, Clock, Trash2, Play, Pause, X, Image as ImageIcon, Ban, Shield, UserMinus, AlertTriangle, Flag, Copy, Reply, Users, LogOut, Bell, Search, Info, Camera, Music, ChevronUp, ChevronDown } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { Avatar } from '../components/ui/avatar';
import { useSessionStore } from '../stores/useSessionStore';
import { AmigoRoomProfileScreen } from './AmigoRoomProfileScreen';
...
interface Message {
  ...
  senderAvatar?: string; // Added for groups
  timestamp: Date;
...
interface AmigoGroupChatScreenProps {
  chatId: string;
  onBack: () => void;
}
...
const MOCK_GROUP_MESSAGES: Message[] = [
  {
    id: '1',
    text: 'Hey team! Just a reminder about the meeting at 3 PM.',
    senderId: 'user1',
  ...
  {
    id: '2',
    text: 'Thanks Alex! I will be there.',
    senderId: 'user2',
  ...
  {
    id: '3',
    text: 'Can someone share the agenda?',
    senderId: 'user3',
  ...
  {
    id: 'doc1',
    text: 'Project_Beta_Agenda.pdf',
    senderId: 'me',
    timestamp: new Date(Date.now() - 1000 * 60 * 45),
    status: 'seen',
    type: 'file',
    fileName: 'Project_Beta_Agenda.pdf',
  },
  {
    id: '5',
    text: 'Perfect, thanks!',
...
export const AmigoGroupChatScreen = ({ chatId, onBack }: AmigoGroupChatScreenProps) => {
  ...
  // Consistency Updates: New State for Rich Input
  ...
  if (showProfile) {
    return (
      <AmigoRoomProfileScreen 
        onBack={() => setShowProfile(false)}
        chatId={chatId}
        name="Team Awesome"
        description="Official team channel for project discussions and updates. 🚀"
        membersCount={12}
      />
    );
  }
  ...
  return (
    <div className={cn("flex h-screen w-full flex-col overflow-hidden transition-colors duration-300 relative", themeStyles.bg)}>
        ...
        <div className="flex-none px-5 pt-4 pb-2 z-20">
            ...
            <div className="flex items-center gap-2">
                ...
                <DropdownMenuContent align="end" className={cn("z-50 min-w-[160px]", isDarkMode ? "bg-[#1A1A2E] border-white/10 text-white" : "bg-white border-gray-200")}>
                    ...
                    <Info className="w-4 h-4 mr-2" /> Room Info
                    </DropdownMenuItem>
                    <DropdownMenuItem className="cursor-pointer" onClick={() => setIsSearching(true)}>
                    <Search className="w-4 h-4 mr-2" /> Search in Chat
                    </DropdownMenuItem>
                    <DropdownMenuItem className="cursor-pointer" onClick={() => setDeleteChatDialogOpen(true)}>
                    <Trash2 className="w-4 h-4 mr-2 text-red-500" /> Clear Chat
                    </DropdownMenuItem>
                    <DropdownMenuSeparator className={isDarkMode ? "bg-white/10" : "bg-gray-200"} />
                    <DropdownMenuItem className="cursor-pointer text-red-500" onClick={() => setReportRoomDialogOpen(true)}>
                    <Flag className="w-4 h-4 mr-2" /> Report Room
                    </DropdownMenuItem>
                    <DropdownMenuItem className="cursor-pointer text-red-500" onClick={() => setLeaveGroupDialogOpen(true)}>
            ...
            >
                <div className="relative">
                    <Avatar name="Team Awesome" size="md" className="w-[54px] h-[54px]" />
                    <div className={cn(
                  ...
                  >
                    {!isMe && (
                      <div className="flex-shrink-0 w-8 flex flex-col justify-end">
                        {isLastInGroup ? (
                          <Avatar name={msg.senderName || 'User'} size="sm" className="w-8 h-8" />
                        ) : (
                          <div className="w-8" />
                        )}
                      </div>
                    ...
                    <div className={cn(
                      ...
                      {!isMe && isFirstInGroup && (
                        <span className={cn(
                            "text-[11px] font-medium mb-1 ml-1",
                      ...
                      <div className={cn("flex items-end gap-2 group/bubble", isMe ? "flex-row-reverse" : "flex-row")}>
                         ...
                         <div className="relative flex flex-col">
                            {msg.replyToId && (
                                <div className={cn(
                                    ...
                                    themeMode === 'ghost' ? "bg-purple-500/10 border-purple-400/50 text-white/90" : isDarkMode ? "bg-blue-500/20 border-blue-400/50 text-white/90" : "bg-blue-500/10 border-blue-500/50 text-gray-800",
                                )}>
                                    <div className="font-semibold mb-0.5 opacity-75">
                                        {messages.find(m => m.id === msg.replyToId)?.senderId === 'me' ? 'You' : messages.find(m => m.id === msg.replyToId)?.senderName}
                                    </div>
                                    <div className="line-clamp-1 opacity-90">
                                        {messages.find(m => m.id === msg.replyToId)?.text || 'Message'}
                                    </div>
                                </div>
                            )}
                                    ...
                                    >
                                        ...
                                        {msg.type === 'image' && msg.mediaUrl && (
                                            <div className="w-full pointer-events-none">
                                                <img src={msg.mediaUrl} alt="Shared content" className="max-w-full min-w-[200px] w-auto h-auto object-cover max-h-[400px]" />
                                            </div>
                                        )}
                                        
                                        {msg.type === 'file' && (
                                            <div className={cn("flex items-center gap-3 p-3 pointer-events-none", isMe ? "bg-white/10" : "bg-black/5")}>
                         ...
                         {/* Message Actions Menu Button (Alternative trigger) */}
                         
                      </div>

                      {/* Delivery Status Text (Outside Bubble) */}
                      <AnimatePresence mode="wait">
                        {isMe && !isLastInGroup && msg.status !== 'sending' && (
                          <motion.div
                            key={msg.status}
                    ...
                    <div className="flex items-center justify-between pl-3 border-l-4 border-blue-500 py-2">
                        <div className="flex-1 min-w-0">
                            <p className="text-xs font-bold text-blue-500 mb-0.5">
                                Replying to {replyingTo.senderId === 'me' ? 'Yourself' : replyingTo.senderName || 'User'}
                            </p>
      ...
      {/* Leave Room Dialog - Crowd Chat Animation Style */}
        ...
        <AlertDialogContent className={isDarkMode ? "bg-[#0A0A14] border-white/10 text-white max-w-md" : "bg-white text-gray-900 max-w-md"}>
           ...
           >
              ...
              <AlertDialogHeader className="text-center space-y-3">
                <AlertDialogTitle className="text-xl">Leave Room?</AlertDialogTitle>
      ...
      {/* Clear Chat Dialog - Crowd Chat Animation Style */}
      <AlertDialog open={deleteChatDialogOpen} onOpenChange={setDeleteChatDialogOpen}>
        <AlertDialogContent className={isDarkMode ? "bg-[#0A0A14] border-white/10 text-white max-w-md" : "bg-white text-gray-900 max-w-md"}>
           ...
           >
              ...
              <AlertDialogHeader className="text-center space-y-3">
                <AlertDialogTitle className="text-xl">Clear Chat?</AlertDialogTitle>
                <AlertDialogDescription className={isDarkMode ? "text-[#8B8CAD]" : "text-gray-500"}>
                  This will delete all messages in this room for you. This action cannot be undone.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter className="w-full mt-6 flex-col-reverse sm:flex-row gap-2">
                 <AlertDialogCancel className={isDarkMode ? "bg-[#141422] text-white border-white/10 hover:bg-[#1A1A2E] hover:text-white" : ""}>
      ...
      {/* Report Room Dialog - Crowd Chat Animation Style */}
        ...
        <AlertDialogContent className={isDarkMode ? "bg-[#0A0A14] border-white/10 text-white max-w-md" : "bg-white text-gray-900 max-w-md"}>
           ...
           >
              ...
              <AlertDialogHeader className="text-center space-y-3">
                ...
                <AlertDialogDescription className={isDarkMode ? "text-[#8B8CAD]" : "text-gray-500"}>
                  Report this room for inappropriate content. Our moderation team will review it.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter className="w-full mt-6 flex-col-reverse sm:flex-row gap-2">
                 <AlertDialogCancel className={isDarkMode ? "bg-[#141422] text-white border-white/10 hover:bg-[#1A1A2E] hover:text-white" : ""}>
                 ...
                 <AlertDialogAction 
                    onClick={() => setReportRoomDialogOpen(false)} 
        ...
        <AlertDialogContent className={isDarkMode ? "bg-[#0A0A14] border-white/10 text-white max-w-md" : "bg-white text-gray-900 max-w-md"}>
           ...
           >
              ...
              <AlertDialogHeader className="text-center space-y-3">
                ...
                {messageToReport && (
                    <div className={cn("p-3 rounded-lg text-sm italic mt-2 text-left w-full", isDarkMode ? "bg-white/5 text-white/80" : "bg-gray-100 text-gray-700")}>
                        "{messageToReport.text || 'Media Attachment'}"
                    </div>
                )}
              </AlertDialogHeader>
              <AlertDialogFooter className="w-full mt-6 flex-col-reverse sm:flex-row gap-2">
                 <AlertDialogCancel className={isDarkMode ? "bg-[#141422] text-white border-white/10 hover:bg-[#1A1A2E] hover:text-white" : ""} onClick={() => setMessageToReport(null)}>
                    Cancel
                 </AlertDialogCancel>
                 <AlertDialogAction 
                    onClick={() => {
